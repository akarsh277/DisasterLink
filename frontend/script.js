...
function showToast(message, type = 'info') {
  ...
  removeTimeout = setTimeout(dismiss, 6000);
}
...
function initLeafletMap(mapId, reports) {
  ...
  reports.forEach(r => {
    ...
    if (r.severity === 'Critical' || r.severity === 'High') {
      ...
      zone.bindTooltip(`${(radius / 1000).toFixed(0)}km Radius`, { permanent: true, direction: 'top', className: 'radius-label', offset: [0, -10] });
...
function handleLiveEvent(msg) {
  ...
  const data = msg.data;
  const isHomePage = !!document.getElementById('homeStatGrid');

  // Silence all WebSocket toasts on the public landing/home page to avoid user clutter,
  // but still allow volunteer status updates to refresh home page counts.
  if (isHomePage) {
    if (tType === 'VOLUNTEER_UPDATE' && data.status === 'COMPLETED') {
      loadHomeDashboard();
    }
    return;
  }
...
function initAdminAuth() {
  ...
  document.querySelectorAll('.admin-tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.admin-section').forEach(sec => sec.classList.remove('admin-section-active'));
      const targetSec = document.getElementById(target);
      if (targetSec) {
        targetSec.classList.add('admin-section-active');
        // Trigger analytics charts when that tab becomes visible
        if (target === 'tab-analytics') renderAdminCharts();
      }
    });
  });
...
let _adminChartData = null;
let _chartsRendered = false;

function storeChartData(reports, vols) {
  _adminChartData = { reports, vols };
  // If the analytics tab is currently active, render now
  if (document.getElementById('tab-analytics')?.classList.contains('admin-section-active')) {
    renderAdminCharts();
  }
}

function renderAdminCharts() {
  if (!_adminChartData || typeof Chart === 'undefined') return;
  if (_chartsRendered) return; // avoid double-render
  _chartsRendered = true;

  const { reports, vols } = _adminChartData;
  ...
  // 1. Incident Types Doughnut
  const typeCount = {};
  reports.forEach(r => { typeCount[r.disaster_type] = (typeCount[r.disaster_type] || 0) + 1; });
  const typeColors = ['#3b82f6', '#ef4444', '#f59e0b', '#10b981', '#8b5cf6', '#ec4899', '#06b6d4'];
  ...
  if (ctx1) {
    new Chart(ctx1, {
      ...
      data: {
        ...
        datasets: [{ data: Object.values(typeCount), backgroundColor: typeColors.slice(0, Object.keys(typeCount).length), borderWidth: 0, hoverOffset: 6 }]
      },
      options: { ...chartDefaults, cutout: '65%', responsive: true, maintainAspectRatio: false }
    });
  }

  // 2. Severity Bar Chart
  const sevOrder = ['Critical', 'High', 'Medium', 'Low'];
  const sevColors = ['#ef4444', '#f97316', '#ca8a04', '#16a34a'];
  const sevCounts = sevOrder.map(s => reports.filter(r => r.severity === s).length);
  ...
  if (ctx2) {
    new Chart(ctx2, {
      ...
      data: {
        labels: sevOrder,
        datasets: [{ label: 'Reports', data: sevCounts, backgroundColor: sevColors, borderRadius: 6, borderWidth: 0 }]
      },
      options: {
        ...chartDefaults,
        ...
        scales: {
          ...
          y: { ticks: { color: '#64748b', stepSize: 1 }, grid: { color: 'rgba(255,255,255,0.06)' }, beginAtZero: true }
        },
  ...
  // 3. Status Doughnut
  const statusColors = { Open: '#ef4444', Resolving: '#f59e0b', Resolved: '#10b981' };
  const statusCount = { Open: 0, Resolving: 0, Resolved: 0 };
  ...
  if (ctx3) {
    new Chart(ctx3, {
      ...
      data: {
        ...
        datasets: [{ data: Object.values(statusCount), backgroundColor: Object.values(statusColors), borderWidth: 0, hoverOffset: 6 }]
      },
      options: { ...chartDefaults, cutout: '65%', responsive: true, maintainAspectRatio: false }
    });
  }

  // 4. Volunteer Skills Horizontal Bar
  const skillCount = {};
  vols.forEach(v => { skillCount[v.skill || 'Other'] = (skillCount[v.skill || 'Other'] || 0) + 1; });
  const ctx4 = document.getElementById('chartSkills');
  if (ctx4) {
    new Chart(ctx4, {
      type: 'bar',
      data: {
        labels: Object.keys(skillCount),
        datasets: [{ label: 'Volunteers', data: Object.values(skillCount), backgroundColor: '#3b82f688', borderColor: '#3b82f6', borderWidth: 1, borderRadius: 6 }]
      },
      options: {
        ...chartDefaults,
        indexAxis: 'y',
        responsive: true, maintainAspectRatio: false,
        scales: {
          x: { ticks: { color: '#64748b', stepSize: 1 }, grid: { color: 'rgba(255,255,255,0.06)' }, beginAtZero: true },
...
async function loadAdminDash() {
  ...
  try {
    ...
    window._adminLastReports = reports; // Store for analytics
    ...
    tblWrap.innerHTML = `
      <div class="table-wrapper">
        <table>
          ...
          <tbody>
            ${reports.length === 0 ? `<tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--text-muted);">No reports registered</td></tr>` : reports.map(r => {
      ...
      return `
                <tr style="${rowStyle}">
                  ...
                  <td>
                    ${r.reporter_name === 'GDACS System' ? `<span class="badge" style="background: rgba(14,165,233,0.15); color: #7dd3fc; border: 1px solid rgba(14,165,233,0.3); font-size:0.7rem; margin-right:0.4rem; padding:0.1rem 0.3rem; vertical-align: middle;">GDACS</span>` : ''}
                    <span style="vertical-align: middle;">${r.description || '—'}</span>
                  </td>
                  <td>
        ...
        cluWrap.innerHTML = `
          <div class="table-wrapper">
            <table>
              ...
              <tbody>
                ${clusters.length === 0 ? `<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--text-muted);">No clusters detected</td></tr>` : clusters.map(c => `
                  <tr>
    ...
    aWrap.innerHTML = `
      <div class="table-wrapper">
        <table style="font-size:0.9rem">
          ...
          <tbody>${alerts.length === 0 ? `<tr><td colspan="4" style="text-align:center;padding:2rem;color:var(--text-muted);">No alerts registered</td></tr>` : alerts.map(a => `<tr><td>${severityBadge(a.severity)}</td><td>${a.location}</td><td>${a.message}</td>
          <td><button class="table-action-btn" onclick="deleteEntity('alerts', ${a.id})" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button></td></tr>`).join('')}</tbody>
        </table>
      </div>`;
  } catch (e) { }
    ...
    cWrap.innerHTML = `
      <div class="table-wrapper">
        <table style="font-size:0.9rem">
          ...
          <tbody>${camps.length === 0 ? `<tr><td colspan="4" style="text-align:center;padding:2rem;color:var(--text-muted);">No camps registered</td></tr>` : camps.map(c => `<tr>
            <td style="font-weight:600">${c.camp_name}</td><td>${c.location}</td>
            <td style="font-weight:700;">${c.capacity}</td>
            <td><button class="table-action-btn" onclick="deleteEntity('camps', ${c.id})" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button></td>
          </tr>`).join('')}</tbody>
        </table>
      </div>`;
  } catch (e) { }
  ...
  try {
    ...
    vWrap.innerHTML = `
      <div class="table-wrapper">
        ...
        ? `<span title="${v.latitude.toFixed(4)}, ${v.longitude.toFixed(4)}" style="color:var(--success);display:inline-flex;align-items:center;gap:0.35rem;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg> <span style="font-family:var(--font-mono);font-size:0.8rem">${v.latitude.toFixed(2)},${v.longitude.toFixed(2)}</span></span>`
        : '<span style="color:var(--text-muted);font-size:0.8rem;display:inline-flex;align-items:center;gap:0.35rem;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> No GPS Data</span>'}
            </td>
    ...
    if (window.appMap) {
      ...
      vols.forEach(v => {
        if (v.latitude && v.longitude && v.assigned_report_id) {
          ...
          circle.bindPopup(`<strong>${v.name}</strong><br/>Skill: ${v.skill}<br/>Status: <b>${v.volunteer_status || 'Assigned'}</b>`);
          window.volMarkers.push(circle);
        }
      });
    }

    if (window._adminLastReports) storeChartData(window._adminLastReports, vols);
  } catch (e) { }
...
async function deleteEntity(endpoint, id, btnEl) {
  showConfirm(`Are you sure you want to delete this ${endpoint.slice(0, -1)}? This action cannot be undone.`, async () => {
    try {
      ...
      if (btnEl) {
        ...
        // No btn reference — reload the dash to reflect changes
        setTimeout(() => loadAdminDash(), 400);
      }
      if (endpoint === 'camps') {
    ...
    aWrap.innerHTML = alerts.length === 0 ? `<p>No alerts.</p>` : `
      <div class="table-wrapper">
        <table style="font-size:0.9rem">
          ...
          <td><button class="table-action-btn" onclick="deleteEntity('alerts', ${a.id}, this)" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button></td></tr>`).join('')}</tbody>
        </table>
      </div>`;
  } catch (e) { }
}
      ...
      <div class="table-wrapper">
        <table style="font-size:0.9rem">
          ...
          <tbody>${camps.map(c => `<tr>
            <td style="font-weight:600">${c.camp_name}</td><td>${c.location}</td>
            <td style="font-weight:700;">${c.capacity}</td>
            <td><button class="table-action-btn" onclick="deleteEntity('camps', ${c.id}, this)" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button></td>
          </tr>`).join('')}</tbody>
        </table>
      </div>`;
  } catch (e) { }
...
async function deleteResource(id) {
  showConfirm('Remove this supply entry? This cannot be undone.', async () => {
    try {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnDonate && donationModal) {
    ...
    if (btnCancelDonation) {
      btnCancelDonation.addEventListener('click', closeModal);
    }
    ...
    if (donationForm) {
      donationForm.addEventListener('submit', async (e) => {
        ...
        const payload = {
          ...
          quantity: parseInt(document.getElementById('donationQty').value) || null,
          item_details: document.getElementById('donationDetails').value.trim() || null
        };
        ...
        <div style="background:rgba(255,255,255,0.03);border-left:4px solid ${badgeColor};padding:1.25rem;margin-bottom:1rem;border-radius:4px;box-shadow:0 4px 6px rgba(0,0,0,0.1);">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem;">
            ...
            <span style="font-size:0.8rem;color:var(--warning);font-weight:bold;background:rgba(245,158,11,0.1);padding:0.2rem 0.5rem;border-radius:4px;">
              Wind: ${pred.wind_speed_kmh} km/h ${pred.wind_direction}
            </span>
          </div>
...
async function loadAdminDonations() {
  const wrapper = document.getElementById('adminDonationsTableWrapper');
  if (!wrapper) return;
  ...
  try {
    ...
    if (donations.length === 0) {
      wrapper.innerHTML = `<p style="padding:2rem; text-align:center; color:var(--text-secondary);">No relief donations registered yet.</p>`;
      return;
    }
        ...
        <table>
          ...
          <tbody>
            ${donations.map(d => {
              ...
              const rowStyle = isReceived ? 'opacity:0.5;' : '';
              return `
                <tr style="${rowStyle}">
                  <td>#${d.id}</td>
                  <td style="font-weight:600">${d.donor_name}</td>
                  <td>${d.donor_phone || '—'}</td>
                  <td><span class="badge badge-other">${d.donation_type}</span></td>
                  <td style="font-weight:700">${d.quantity || '—'}</td>
                  <td>${d.item_details || '—'}</td>
                  <td>
                    <span class="badge ${isReceived ? 'badge-low' : 'badge-high'}">${d.status}</span>
                  </td>
                  <td>
                    ${!isReceived ? `
                      <button class="btn btn-sm btn-primary" onclick="receiveDonation(${d.id}, this)" style="padding:0.25rem 0.5rem; font-size:0.75rem;">
                        Mark Received
                      </button>
                    ` : 'Received'}
                  </td>
    ...
    wrapper.innerHTML = `<p style="padding:2rem; text-align:center; color:var(--danger);">Failed to load donations.</p>`;
  }
}

async function receiveDonation(id, btnEl) {
  if (btnEl) {
    btnEl.disabled = true;
  ...
  try {
    ...
    if (btnEl) {
      ...
      btnEl.textContent = 'Mark Received';
    }
  }
}
